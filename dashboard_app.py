...
def download_corrected(report_id):
    ...
    if report_id.startswith('db_'):
        ...
        file_content = db_report.get('corrected_content')
        if not file_content:
            return "File content not found in database", 404
        
        # Create buffer from database content
        ...
        filename = db_report['filename']
        if db_report.get('corrections_applied', 0) > 0:
            # Add _CORRECTED suffix if corrections were applied
            filename = filename.replace('.txt', '_CORRECTED.txt').replace('.xml', '_CORRECTED.xml')
